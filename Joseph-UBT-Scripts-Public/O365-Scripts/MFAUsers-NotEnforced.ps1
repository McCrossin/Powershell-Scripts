...
foreach ($box in $mailboxes) 
{
    ...
    if ($isValid -and ($IsShared -eq $false) -and $IsLicensed){
        ...
        $object | export-csv -Path C:\temp\MFAReport.csv -Append

        LogWrite ("Email: " + $Email + "Name: " + $Name + "DataBase Location: " + $DataBaseLocation)

    }else{
         LogError ($Email + " Is an SharedMailbox or an Offboarded user")
    }
}
